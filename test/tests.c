...
CHEAT_TEST(token_init_if,
    token_t *tok = init_token(IF, "if");
...
CHEAT_TEST(token_init_sub_all,
    token_t *tok = init_token_sub(IDENTIFIER, "function", 8);
    cheat_assert(tok->type == IDENTIFIER);
    cheat_assert_string(tok->value, "function");
)
CHEAT_TEST(token_init_sub,
    token_t *tok = init_token_sub(IDENTIFIER, "function", 4);
    cheat_assert(tok->type == IDENTIFIER);
    cheat_assert_string(tok->value, "func");
)
CHEAT_TEST(token_init_sub_empty,
    token_t *tok = init_token_sub(IDENTIFIER, "function", 0);
    cheat_assert(tok->type == IDENTIFIER);
    cheat_assert_string(tok->value, "");
)

CHEAT_TEST(token_to_str,
    token_t *tok = init_token(IDENTIFIER, "function");
    cheat_assert_string(token_to_str(tok), 
        "<token type='IDENTIFIER' value='function'>");
)
...
CHEAT_TEST(token_to_str_inv,
    token_t *tok = init_token((tokentype)-1, "");
    cheat_assert_string(token_to_str(tok), 
